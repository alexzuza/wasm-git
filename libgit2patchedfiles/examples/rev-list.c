...
struct rev_list_options {
    int left_right;
};

static int revwalk_parse_options(git_sort_t *sort, struct rev_list_options* opt, struct args_info *args);
...
int lg2_rev_list(git_repository *repo, int argc, char **argv)
{
	...
	git_oid oid;
	git_sort_t sort;
	struct rev_list_options opt;
	char buf[GIT_OID_HEXSZ+1];

	check_lg2(revwalk_parse_options(&sort, &opt, &args), "parsing options", NULL);

	// rev-list --left-right sha1..sha2
	if (opt.left_right && argc == 3) {
	    git_revspec revspec;
	    int error = 0;
	    if (error = git_revparse(&revspec, repo, argv[2]))
		    return error;

	    static size_t ahead;
	    static size_t behind;
	    check_lg2(git_graph_ahead_behind(
		    &ahead, &behind, repo,
		    git_object_id(revspec.from),
		    git_object_id(revspec.to)), "ahead behind", NULL);
...
{
	fprintf(stderr, "rev-list [--git-dir=dir] [--topo-order|--date-order] [--reverse] [--left-right] <revspec>\n");
	exit(-1);
}
...
static int revwalk_parse_options(git_sort_t *sort, struct rev_list_options* opt, struct args_info *args)
{
	...
	*sort = GIT_SORT_NONE;
	...
	for (args->pos = 1; args->pos < args->argc; ++args->pos) {
		...
		} else if (!strcmp(curr, "--left-right"))
		    opt->left_right = 1;
		else {
			break;
		}
